...
var UWindowGridColumn Server, Ping, MapName, Players, Games, SortByColumn;
var bool bSortDescending;
var localized string ServerName, PingName, MapNameName, PlayersName, GamesName;
...
{
	Server	= AddColumn(ServerName, 200);
	Ping	= AddColumn(PingName, 25);
	MapName	= AddColumn(MapNameName, 90);
	Players	= AddColumn(PlayersName, 30);
	Games	= AddColumn(GamesName, 75);
...
function JoinServer(UBrowserServerList Server)
{
	...
	{
		GetPlayerOwner().ClientTravel("rune://"$Server.IP$":"$Server.GamePort$UBrowserServerListWindow(GetParent(class'UBrowserServerListWindow')).URLAppend, TRAVEL_Absolute, false);
...
function int ByGames(UBrowserServerList T, UBrowserServerList B)
{
	local int Result;
