...
// RUNE: Compiles out english for single language installs
#ifndef COMPILE_OUT_ENGLISH
#define COMPILE_OUT_ENGLISH 1
#endif

// RUNE: Defined for Rune Add-on, Halls of Valhalla
#define HALLSOFVALHALLA 0
...
class WFilerPageFolder : public WWizardPage, public FControlSnoop
{
	...
	WWizardPage* GetNext()
	{
...
#if !HALLSOFVALHALLA // Don't check requirements for HOV
		...
		{
#if HALLSOFVALHALLA
			// Get language version for HOV
			FString oldLang;
			GConfig->GetString(TEXT("Engine.Engine"), TEXT("Language"), oldLang, 
				*(NewFolder * TEXT("\\System\\Rune.ini")));
			if(oldLang != TEXT(""))
				Manager->Language = oldLang;
			else
				Manager->Language = TEXT("int");

			UObject::SetLanguage(*Manager->Language);
			GConfig->SetString( TEXT("Setup"), TEXT("Language"), *Manager->Language, *Manager->ConfigFile );

//			FString Title = FString::Printf(TEXT("LANGUAGE SETTING"));
//			FString Msg   = FString::Printf(TEXT("Language=%s NewFolder=%s : Registry=%s\nDefaultFolder=%s\n "), 
//				*Manager->Language, *NewFolder, *Manager->RegistryFolder, *Manager->DefaultFolder);
...
class WFilerPageWelcome : public WWizardPage
{
	...
	void OnInitDialog()
	{
		...
		{
#if COMPILE_OUT_ENGLISH
//			FString Title = FString::Printf( TEXT("COMPILE_OUT_ENGLISH") );
//			FString Msg   = FString::Printf( TEXT("Manager->Language=%s Result(%d).Object=%s\n "), *Manager->Language, i, *Results(i).Object );
//			MessageBox( *Owner, *Msg, *Title, MB_OK );

			// RUNE: Single language install support (gets rid of english if non-english install)
			if (appStricmp(*Manager->Language, TEXT("int")) && !appStricmp(*Results(i).Object, TEXT("int")))
			{	// Setup Definition of non-english and this one is english, skip
				new(LanguageNames)FString( TEXT("Skipping") );	// Must pad list with something
				continue;
			}
#endif
...
#else // Halls of Valhalla sets the language to whatever Rune language was installed
		UObject::SetLanguage(*Manager->Language);
		GConfig->SetString( TEXT("Setup"), TEXT("Language"), *Manager->Language, *Manager->ConfigFile );
#endif
