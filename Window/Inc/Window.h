...
class WINDOW_API WCoolButton : public WButton
{
	...
	void OnDrawItem( DRAWITEMSTRUCT* Item )
	{
		...
		if( FrameFlags & CBFF_UrlStyle )
		{
			...
			SelectObject( Item->hDC, hFontText );
...
public:
	...
	void OnItemSetFocus()
	{
		...
		&&	!(Property->PropertyFlags & CPF_EditConst) )
		{
			...
			||	(Property->IsA(UNameProperty::StaticClass()) && (Name==NAME_Orders || Name == NAME_TriggerOrders || Name == NAME_AlertOrders))	//RUNE
			||  (Cast<UClassProperty>(Property) && appStricmp(*Property->Category,TEXT("Drivers"))==0) )
			{
				...
				else if( Property->IsA(UNameProperty::StaticClass()) && (Name==NAME_Orders || Name == NAME_TriggerOrders || Name == NAME_AlertOrders))
				{ // RUNE:  Orders
					TArray<FName> States;
					GetStates( States );
					ComboControl->AddString( *FName(NAME_None) );
			...
			||	(Property->IsA(UNameProperty  ::StaticClass()) && (Name!=NAME_InitialState && Name!=NAME_Orders && Name!=NAME_TriggerOrders && Name!=NAME_AlertOrders))	//RUNE
