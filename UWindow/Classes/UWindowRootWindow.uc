...
function DrawMouse(Canvas C) 
{
	...
	DrawDropShadowed(C, 
		MouseX * GUIScale - MouseWindow.Cursor.HotX,
		MouseY * GUIScale - MouseWindow.Cursor.HotY,
		GetPlayerOwner().GetItemName(string(MouseWindow))$" "$int(MouseX * GUIScale)$", "$int(MouseY * GUIScale)$" ("$int(X)$", "$int(Y)$")");

	DrawDropShadowed(C, 
		MouseX * GUIScale - MouseWindow.Cursor.HotX,
		MouseY * GUIScale - MouseWindow.Cursor.HotY + 10,
		" Parent:      " @ MouseWindow.ParentWindow);

	DrawDropShadowed(C, 
		MouseX * GUIScale - MouseWindow.Cursor.HotX,
		MouseY * GUIScale - MouseWindow.Cursor.HotY + 20,
		" FirstChild:  " @ MouseWindow.FirstChildWindow);

	DrawDropShadowed(C, 
		MouseX * GUIScale - MouseWindow.Cursor.HotX,
		MouseY * GUIScale - MouseWindow.Cursor.HotY + 30,
		" LastChild:   " @ MouseWindow.LastChildWindow);

	DrawDropShadowed(C, 
		MouseX * GUIScale - MouseWindow.Cursor.HotX,
		MouseY * GUIScale - MouseWindow.Cursor.HotY + 40,
		" NextSibling: " @ MouseWindow.NextSiblingWindow);

	DrawDropShadowed(C, 
		MouseX * GUIScale - MouseWindow.Cursor.HotX,
		MouseY * GUIScale - MouseWindow.Cursor.HotY + 50,
		" PrevSibling: " @ MouseWindow.PrevSiblingWindow);

	DrawDropShadowed(C, 
		MouseX * GUIScale - MouseWindow.Cursor.HotX,
		MouseY * GUIScale - MouseWindow.Cursor.HotY + 60,
		" ActiveWindow:" @ MouseWindow.ActiveWindow);

	DrawDropShadowed(C, 
		MouseX * GUIScale - MouseWindow.Cursor.HotX,
		MouseY * GUIScale - MouseWindow.Cursor.HotY + 70,
		" Root:        " @ MouseWindow.Root);

	DrawDropShadowed(C, 
		MouseX * GUIScale - MouseWindow.Cursor.HotX,
		MouseY * GUIScale - MouseWindow.Cursor.HotY + 80,
		" OwnerWindow: " @ MouseWindow.OwnerWindow);

	DrawDropShadowed(C, 
		MouseX * GUIScale - MouseWindow.Cursor.HotX,
		MouseY * GUIScale - MouseWindow.Cursor.HotY + 90,
		" ModalWindow: " @ MouseWindow.ModalWindow);

	DrawDropShadowed(C, 
		MouseX * GUIScale - MouseWindow.Cursor.HotX,
		MouseY * GUIScale - MouseWindow.Cursor.HotY + 100,
		" IsActive:   " @ MouseWindow.IsActive());

	DrawDropShadowed(C, 
		MouseX * GUIScale - MouseWindow.Cursor.HotX,
		MouseY * GUIScale - MouseWindow.Cursor.HotY + 110,
		" Dimensions: " @ MouseWindow.WinLeft @ MouseWindow.WinTop @ MouseWindow.WinWidth @ MouseWindow.WinHeight);

	*/
}

function DrawDropShadowed(Canvas C, float X, float Y, string text)
{
	C.SetColor(0,0,0);
	C.SetPos(X-1,Y-1);
	C.DrawText(text);

	C.SetColor(255,255,0);
	C.SetPos(X,Y);
	C.DrawText(text);
}
...
function ComputeGuiScale(float width, float height)
{
	local float aspect;
	...
	if (aspect < 0.75)
	{
		GUIScale = height/480.0;
	}
	else
	{
		GUIScale = width/640.0;
	}
