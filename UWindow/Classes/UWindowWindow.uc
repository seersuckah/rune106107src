...
var float				WindowAlpha;			// RUNE: Alpha Channel
...
function WindowEvent(WinMessage Msg, Canvas C, float X, float Y, int Key) 
{
	...
	case WM_MMouseUp:
		...
		break;	
	case WM_KeyDown:
//log(name@"WindowEvent propagating  :"$Name@"Msg="$Msg@"Key="$Key);
		if(!PropagateKey(Msg, C, X, Y, Key))
		{
//log(name@"**WindowEvent propagated to:"$Name@Msg@"Key="$Key);
			KeyDown(Key, X, Y);
		}
		break;	
	case WM_KeyUp:
...
final function PaintClients(Canvas C, float X, float Y)
{
	...
	while(Child != None)
	{
		...
		C.DrawColor.G = 255;
		C.DrawColor.B = 255;
		if (Child.WindowAlpha != 1.0 || Root.Console.MenuTranslucency != 1.0)
		{
			C.Style = GetPlayerOwner().ERenderStyle.STY_AlphaBlend;
			C.AlphaScale = Child.WindowAlpha*Root.Console.MenuTranslucency;
		}
		else
		{
...
//log(name@"propogatekey for msg:"$Msg);
...
//log("   child="$Child@"Active="$bUWindowActive@"LeaveOnScreen="$Child.bLeaveOnscreen@"AcceptsFocus="$Child.bAcceptsFocus);
...
final function UWindowWindow FindChildWindow(class<UWindowWindow> ChildClass, optional bool bExactClass)
{
	...
	for(Child = LastChildWindow;Child != None;Child = Child.PrevSiblingWindow)
	{
		...
		Found = Child.FindChildWindow(ChildClass, bExactClass);
