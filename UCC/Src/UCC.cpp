...
int main( int argc, char* argv[] )
{
	...
	// RUNE: Tell skelmodels we're in UCC so we can avoid accessing the cache (conforming maps)
	GUglyHackFlags|=2;
...
#endif
	{
		...
		*CmdLinePtr = 0;
		...
		// Init engine core.
//RUNE
		appInit( TEXT("Rune"), CmdLine, &Malloc, &Log, &Error, &Warn, &FileManager, FConfigCacheIni::Factory, 1 );
	...
	GUglyHackFlags&=~2;	// RUNE: No longer in UCC
